...
rundir = "ChainStormDesignV2_2_warmdown_prevoptstate"
mkpath(rundir)
...
model = ChainStormDesignV2(384, 6, 6, 6)
file = "/home/murrellb/ProtModels/ChainStormConditioning/ChainStorm.jl/sandbox/ChainStormDesignV2_2/checkpoint_15.jld2"
Flux.loadmodel!(model, JLD2.load(file, "model_state"))
model = model |> device
opt_state = JLD2.load(file, "opt_state") |> device;

prev_lr = Float32(opt_state.layers.cond_t_encoding.weight.rule.eta)
sched = linear_decay_schedule(prev_lr, 0.000000001f0, 3400) #Two epoch warmdown
...
for epoch in 16:100
    ...
    checkpoint_path = "$(rundir)/checkpoint_warmdown_$(epoch).jld2"
