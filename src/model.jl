...
struct ChainStormDesign{L}
    layers::L
end
Flux.@layer ChainStormDesign
function ChainStormDesign(dim, depth, f_depth)
    layers = (;
        depth = depth,
        f_depth = f_depth,
        t_rff = RandomFourierFeatures(1 => dim, 1f0),
        ...
        AAdecoder = Chain(StarGLU(dim, 3dim), Dense(dim => 21, bias=false)),
        #Conditioning layers
        design_rff = RandomFourierFeatures(3 => 64, 1f0),
        design_project = Dense(64 => 32, bias=false),
    )
    return ChainStormDesign(layers)
end

function (fc::ChainStormDesign)(t, Xt, chainids, resinds; sc_frames = nothing, pair_conditioning = nothing)
    l = fc.layers
    pmask = Flux.Zygote.@ignore self_att_padding_mask(Xt[1].lmask)
    pre_z = Flux.Zygote.@ignore l.pair_rff(pair_encode(resinds, chainids))
    pair_feats = l.pair_project(pre_z)
    if !isnothing(pair_conditioning)
        pair_feats = pair_feats .+ l.design_project(l.design_rff(pair_conditioning)) .* pair_conditioning[3:3,:,:,:] #<- inductive bias where 0 means no change.
    end
    t_rff = Flux.Zygote.@ignore l.t_rff(t)
    cond = reshape(l.cond_t_encoding(t_rff), :, 1, size(t,2))
    frames = Translation(tensor(Xt[1])) ∘ Rotation(tensor(Xt[2]))
...
function (block::Onion.AdaTransformerBlock)(x, cond, rope, mask, scale)
    h = x + block.attention(block.attention_norm(x, cond), 0, rope, mask) / scale
    out = h + block.feed_forward(block.ffn_norm(h, cond)) / scale
    return out
end

struct ChainStormDesignV2{L}
    layers::L
end
Flux.@layer ChainStormDesignV2
function ChainStormDesignV2(dim, depth, f_depth, t_depth)
    layers = (;
        depth = depth,
        f_depth = f_depth,
        t_depth = t_depth,
        ...
        transformers = [Onion.AdaTransformerBlock(dim, dim, 8) for _ in 1:t_depth],
    )
    return ChainStormDesignV2(layers)
end

function (fc::ChainStormDesignV2)(t, Xt, chainids, resinds; sc_frames = nothing, sc_aa = nothing, pair_conditioning = nothing)
    l = fc.layers
    pmask = Flux.Zygote.@ignore self_att_padding_mask(Xt[1].lmask)
    pre_z = Flux.Zygote.@ignore l.pair_rff(pair_encode(resinds, chainids))
    pair_feats = l.pair_project(pre_z)
    ...
    for i in 1:l.depth
        ...
        if i <= l.t_depth
            x = Flux.Zygote.checkpointed(l.transformers[i], x, cond, nothing, pmask, l.t_depth)
        end
        x = Flux.Zygote.checkpointed(ipa, l.ipa_blocks[i], frames, x, pair_feats, cond, pmask)
        if i > l.depth - l.f_depth
            frames = l.framemovers[i - l.depth + l.f_depth](frames, x, t = t)
