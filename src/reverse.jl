...
function reverse_gen(
    ...
    for (s₁, s₂) in zip(steps, steps[begin+1:end])
        ...
        if t < snap_time
            ...
            push!(record, (1-s₂, deepcopy(Xₜ), fakeX̂₁)) #records all the steps
        else
            push!(record, (1-s₂, deepcopy(Xₜ), deepcopy(X̂₁))) #records all the steps
        end
        tracker(1-t, Xₜ, X̂₁)
    end
    return Xₜ
...
function bind_gen(
    ...
    for (s₁, s₂) in zip(steps, steps[begin+1:end])
        ...
        pop!(record)
        tensor(Xₜ[1])[:, :, 1:size(tensor(old_Xₜ[1]), 3), :] .= tensor(old_Xₜ[1])
        tensor(Xₜ[2])[:, :, 1:size(tensor(old_Xₜ[2]), 3), :] .= tensor(old_Xₜ[2])
        tensor(Xₜ[3]).indices[1:size(tensor(old_Xₜ[3]).indices, 1), :] .= tensor(old_Xₜ[3]).indices
        Xₜ = mask(step(P, Xₜ, X̂₁, s₁, s₂), X₀)
        if length(record) == 1
            tensor(Xₜ[1])[:, :, 1:size(tensor(old_Xₜ[1]), 3), :] .= tensor(old_Xₜ[1])
            tensor(Xₜ[2])[:, :, 1:size(tensor(old_Xₜ[2]), 3), :] .= tensor(old_Xₜ[2])
